...
const VALUES: { icon: LucideIcon; title: string; body: string }[] = [
  { icon: Flame, title: "Seared To Order", body: "Every fillet hits the flat-top the moment you order — never pre-cooked, never sitting." },
  { icon: ChefHat, title: "House-Made Sauces", body: "From smooth B&H mayo to the famous Algerian kick, our sauces are mixed in-house daily." },
  { icon: Leaf, title: "100% Fresh", body: "Crisp lettuce, ripe tomato and toasted buns — quality you can taste in every bite." },
  { icon: Bike, title: "Sub-10 Delivery", body: "A tight kitchen pipeline gets your build to the door before it ever cools down." },
];
    ...
    <div ref={ref} className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
      {VALUES.map((v) => (
        ...
        >
          ...
          <h3 className="mt-5 font-display text-2xl tracking-wide text-cream">{v.title}</h3>
