...
import { saveDealsBlockContentAction } from "@/server/actions/content";
...
export function DealsBlockEditor({ initial }: { initial: DealsBlockContent }) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [saved, setSaved] = useState(false);
  const [form, setForm] = useState<DealsBlockContent>(initial);
  const set = (k: keyof DealsBlockContent, v: string) => setForm((f) => ({ ...f, [k]: v }));

  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    startTransition(async () => {
      await saveDealsBlockContentAction(form);
      setSaved(true);
    ...
    <form onSubmit={submit} className="max-w-2xl space-y-5">
      <div className="flex items-center justify-between gap-3">
        <p className="text-sm text-slate-400 tracking-wide">
          Heading for the &ldquo;Today&rsquo;s Deals&rdquo; strip on the homepage. The deal cards come from
          Deals &amp; Promo Codes.
        </p>
      ...
      <div className="bg-ink-2 rounded-xl border border-white/8 shadow-[0_1px_4px_rgba(0,0,0,0.5)] p-5 space-y-4">
        ...
        <div>
          <label className={labelCls}>&ldquo;View all&rdquo; link label</label>
